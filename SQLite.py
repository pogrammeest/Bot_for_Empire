...
class sqlite():
    ...
    def enter(self,table_name,values):#метод ввода любых значений в любую таблицу.В аргументы ввести имя таблицы с перечислением столбцов и значения в виде кортежа
        count=int()
        for i in values:
            count=count+1
        self.curs.execute(f"INSERT INTO {table_name} VALUES ({(count-1)*'?,'} ?)",values)

    def read(self,argue,table_name):#метод вывода любых значений из любой таблицы. В аргументы ввести выборку и имя таблицы
    ...
    def delete(self,table_name,where):#метод удаления значения с условием.В аргументы ввести название таблицы и условие
